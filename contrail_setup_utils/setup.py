...
class KeepalivedSetup(Setup):
    def fixup_config_files(self):
        ...
        if self._args.openstack_index == 1:
            state = 'MASTER'
            priority = '101'
        vip_str = '_'.join(self._args.openstack_vip.split('.'))
        router_id = self._args.openstack_vip.split('.')[3]
...
class OpenstackGaleraSetup(Setup):
    def fixup_config_files(self):
        ...
        local('sed -i "/innodb_locks_unsafe_for_binlog=1/a #INNODB" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-buffer-pool-size=500M" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-additional-mem-pool-size=1000M" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-flush-log-at-trx-commit=2" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-file-per-table=1" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-read-io-threads=4" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-write-io-threads=4" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-doublewrite=1" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-thread-concurrency=0" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-flush-method = O_DIRECT" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-locks-unsafe-for-binlog=1" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-autoinc-lock-mode=2" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a innodb-stats-on-metadata=0" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a engine-condition-pushdown=1" %s' % wsrep_conf)
        local('sed -i "/#INNODB/a default-storage-engine=innodb" %s' % wsrep_conf)
        local('sed -i "/innodb-buffer-pool-size=500M/a #BUFFERS" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a key_buffer_size = 24M" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a tmp_table_size = 64M" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a max_heap_table_size = 64M" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a max-allowed-packet = 512M" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a skip-name-resolve" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a memlock=0" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a sysdate-is-now=1" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a thread-cache-size=512" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a table-open_cache=1024" %s' % wsrep_conf)
        local('sed -i "/#BUFFERS/a lower-case-table-names=0" %s' % wsrep_conf)
        local('sed -i -e "s#wsrep_slave_threads=*#wsrep_slave_threads=8#g" %s' % wsrep_conf)
        local('sed -i -e "s#wsrep_notify_cmd=*#\#wsrep_notify_cmd=#g" %s' % wsrep_conf)
        local('sed -i -e "s#query_cache_size=*#\#query_cache_size=#g" %s' % wsrep_conf)
        local('sed -i -e "s#query_cache_type=*#\#query_cache_type=#g" %s' % wsrep_conf)
